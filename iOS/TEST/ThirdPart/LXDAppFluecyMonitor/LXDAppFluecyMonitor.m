...
- (void)startMonitoring {
    ...
    dispatch_async(lxd_event_monitor_queue(), ^{
        while (SHAREDMONITOR.isMonitoring) {
            // 监听一次完整的runloop，即监听者收到的通知： kCFRunLoopBeforeWaiting -> kCFRunLoopAfterWaiting -> kCFRunLoopBeforeTimers -> kCFRunLoopBeforeSources -> kCFRunLoopBeforeWaiting
            if (SHAREDMONITOR.currentActivity == kCFRunLoopBeforeWaiting) {
                __block BOOL timeOut = YES;
                dispatch_async(dispatch_get_main_queue(), ^{
                    // 若主线程出现阻塞，则 timeOut 不会立即执行，若超过睡眠设置的时间（超时时间），则判定主线程卡顿
                    timeOut = NO;
